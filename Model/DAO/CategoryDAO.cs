...
namespace Model.DAO
{
    public class CategoryDAO
    {
        ...
        {
            return db.CATEGORY.Where(x => x.Name == name).First();
        }
        ...
        public int GetProductQuantityByManufacturerAndCategory(int idCategory, string nameManufacturer)
        {
            int result = db.PRODUCT.Join(db.MANUFACTURER,
                            p => p.Id_Manufacturer, m => m.Id,
                            (p, m) => new { PRODUCT = p, MANUFACTURER = m })
                            .Where(pm => pm.PRODUCT.Id_Category == idCategory
                            && pm.MANUFACTURER.Name == nameManufacturer).Count();
            return result;
        }
    }
}
